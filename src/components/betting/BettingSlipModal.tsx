...
import { useBetting, BettingSelection } from '@/contexts/BettingContext';
...
export default function BettingSlipModal({ isOpen, onClose }: BettingSlipModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end">
      ...
      {/* Modal */}
      <div className="relative bg-gray-900 text-white w-full max-w-md h-full max-h-[90vh] flex flex-col">
        {/* Header */}
          ...
          {selections.length === 0 ? (
            ...
            <div className="p-4 space-y-4">
              {selections.map((selection, index) => (
                <div key={selection.odd.id} className="bg-gray-800 rounded-lg p-4 relative">
                  {/* Remove button */}
                  <button
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <div className="font-medium text-white">{selection.event.name}</div>
                  ...
                  <div className="mb-3">
                    <div className="text-sm text-gray-400 mb-1">{selection.market.shortName}</div>
                    <div className="font-medium text-white">{selection.odd.name}</div>
                  </div>

                  {/* Odds */}
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-sm text-gray-400">Odd</span>
                    <span className="text-lg font-bold text-blue-400">{selection.odd.price}</span>
                  </div>

                  {/* Stake input */}
                  <div className="mb-3">
                    <label className="block text-sm text-gray-400 mb-1">Valor da aposta</label>
                    <input
                      type="number"
                      placeholder="0.00"
                  ...
                  {/* Potential win */}
                  <div className="text-sm text-gray-400">
                    Ganha: <span className="text-green-400 font-medium">R$ 0.00</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="border-t border-gray-700 p-4 space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Valor total de aposta</span>
                <span className="text-white font-medium">R$ {getTotalStake().toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Ganho total</span>
                <span className="text-green-400 font-medium">R$ {getTotalWin().toFixed(2)}</span>
              </div>
            </div>
